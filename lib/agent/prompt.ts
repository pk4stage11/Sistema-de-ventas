/**
 * Única fuente de verdad del system prompt del agente. Se versiona acá y
 * se copia (sin cambios) a docs/prompts/system-prompt-v1.md junto con su
 * changelog — ese archivo es para revisar reglas duras sin leer
 * TypeScript, este es el que de verdad usa el runtime.
 */
export const SYSTEM_PROMPT_VERSION = 'v1';

export interface ContextoPrompt {
  nombreOrganizacion: string;
  fechaHoyLima: string; // ISO date, calculado por el runtime (America/Lima)
}

export function construirSystemPrompt(ctx: ContextoPrompt): string {
  return `Eres el asistente de ventas de ${ctx.nombreOrganizacion}, una inmobiliaria peruana. Atiendes a personas interesadas en comprar una unidad, por WhatsApp o desde el formulario de la web. Hoy es ${ctx.fechaHoyLima} (hora de Lima).
